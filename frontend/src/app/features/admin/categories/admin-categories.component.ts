...
@Component({
  ...
  imports: [CommonModule, ReactiveFormsModule, ButtonModule, TableModule,
            DialogModule, InputTextModule, InputTextareaModule, ToastModule, ConfirmDialogModule],
...
export class AdminCategoriesComponent implements OnInit {
  ...
  save(): void {
    ...
    request$.subscribe({
      next: () => {
        this.toast.add({ severity: 'success', summary: 'Succès', detail: editing ? 'Catégorie modifiée' : 'Catégorie créée' });
  ...
  confirmDelete(category: Category): void {
    this.confirm.confirm({
      message: this.i18n.currentLang() === 'ar' ? `حذف الفئة "${category.name}"؟` : this.i18n.currentLang() === 'en' ? `Delete category "${category.name}"?` : `Supprimer la catégorie "${category.name}" ?`,
      ...
      accept: () => {
        this.categoryService.delete(category.id).subscribe({
          next: () => {
            this.toast.add({ severity: 'success', summary: this.i18n.currentLang() === 'ar' ? 'تم الحذف' : this.i18n.currentLang() === 'en' ? 'Deleted' : 'Supprimée', detail: category.name });
            this.loadCategories();
          },
