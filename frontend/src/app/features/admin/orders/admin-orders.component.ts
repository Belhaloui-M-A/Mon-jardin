...
import { Order, OrderStatus, ORDER_STATUS_LABELS } from '../../../core/models/models';
...
export class AdminOrdersComponent implements OnInit {
  ...
  statusFlow: { [key: string]: { label: string; value: OrderStatus }[] } = {
    PENDING:   [{ label: 'Confirmer', value: 'CONFIRMED' }, { label: 'Annuler', value: 'CANCELLED' }],
    CONFIRMED: [{ label: 'Expédier', value: 'SHIPPED' }, { label: 'Annuler', value: 'CANCELLED' }],
  ...
  getStatusBg(status: OrderStatus): string {
    const m: Record<OrderStatus,string> = {PENDING:'rgba(251,191,36,.12)',CONFIRMED:'rgba(56,189,248,.12)',SHIPPED:'rgba(167,139,250,.12)',DELIVERED:'rgba(74,222,128,.12)',CANCELLED:'rgba(239,68,68,.12)'};
    return m[status];
  }
  getStatusColor(status: OrderStatus): string {
    const m: Record<OrderStatus,string> = {PENDING:'#fbbf24',CONFIRMED:'#38bdf8',SHIPPED:'#a78bfa',DELIVERED:'#4ade80',CANCELLED:'#f87171'};
    return m[status];
  }
