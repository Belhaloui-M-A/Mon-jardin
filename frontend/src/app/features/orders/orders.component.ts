...
export class OrdersComponent implements OnInit {
  ...
  statusSteps = [
    {
      status: "PENDING",
      key: "orders_status_pending" as keyof Translations,
      icon: "pi pi-clock",
    },
    {
      status: "CONFIRMED",
      key: "orders_status_confirmed" as keyof Translations,
      icon: "pi pi-check",
    },
    {
      status: "SHIPPED",
      key: "orders_status_shipped" as keyof Translations,
      icon: "pi pi-send",
    },
    {
      status: "DELIVERED",
      key: "orders_status_delivered" as keyof Translations,
      icon: "pi pi-home",
    },
  ];
  ...
  cancel(id: number): void {
    this.orderService.cancel(id).subscribe({
      next: (updated) => {
        ...
        this.toast.add({
          severity: "info",
          summary: this.t.orders_status_cancelled,
          detail: `Commande #${id} annulée`,
        });
      },
      error: (err) =>
        this.toast.add({
  ...
  getStatusBorder(status: OrderStatus): string {
    const m: Record<OrderStatus, string> = {
      PENDING: "rgba(251,191,36,.25)",
      CONFIRMED: "rgba(56,189,248,.25)",
      SHIPPED: "rgba(167,139,250,.25)",
      DELIVERED: "rgba(74,222,128,.25)",
